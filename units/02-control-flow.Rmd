...
It is also possible to have the if clause (without _else_)
```{r eval = FALSE}
# just if
if (condition) { 
  expression1 
...
- R provides a family of functions that tend to be more efficient than loops
(i.e. `apply()` functions)
...
- If you know the number to times that something will be done, use `for`
